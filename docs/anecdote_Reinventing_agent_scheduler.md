...
强流程刚立起来的时候，效果好得有点吓人。Codex 一旦戴上 PR reviewer 的帽子，就像变了一个人。

之前它还只是温温和和地规划、总结、提建议；等它真正进入 review 位，立刻开始较真。M2 第一轮 review，它把 4 个 issue 里的 3 个打了回去，而且打得有理有据。

那一刻我明显感觉到，流程改变了 agent 的行为方式。不是模型突然变聪明了，而是角色、责任和交付面变清楚以后，它开始用 reviewer 的方式思考。

但是，反弹也来得很快。

我们当时把 issue / branch / PR 的粒度切得很细，还用了 stacked PR。理论上这很漂亮：边界清楚，review 精确，责任可追踪。现实是，stacked PR 对人类都不算轻松，对两个 agent 加一个半自动人肉调度员来说更微妙。

PR 在 GitHub 上显示 merged，不代表它已经进了 `main`；它可能只是合进了自己的 base branch。于是状态开始变得诡异：issue 看起来合了，Project 看起来流转了，DeepSeek 那边却发现 `main` 上什么都没有。

隔壁 DeepSeek 一脸问号地反复追问：这些 issue 为什么又回来了？反馈在哪里？我应该在哪个分支上改？是不是还有我看不到的 review？

它已经很客气了。如果换成人，估计这时候多少要开始阴阳怪气。

这次事故逼着我和 Codex 一起反思：流程不是越细越好。逐个 issue 批准开工，会把人类重新拖回消息队列的位置；严格隔离每个 issue 的变更，会制造 branch base、PR target、merge destination 这些额外复杂度。流程越精密，越容易让 agent 在局部状态里做出“看起来正确”的动作。

于是我们又往回调了一档：默认不再追求 stacked PR 的精细感，而是改用 Epic integration branch。child issue 仍然有自己的 PR，但先合进 Epic branch，最后由一个 Epic PR 进 `main`。`Ready` 也不再等于“立刻开工”，而是可以表示 Implementer 的有序队列；真正能不能动手，由 issue 里的 `Depends on` 决定。

这就没那么“完美流程”了，但更像 vibe coding 需要的平衡：足够结构化，避免失忆和乱流；又不要精细到把速度和直觉全锁死。甚至可以说，对 vibe coding 来说，fail fast 也许才是正道。
